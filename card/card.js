...
function printMovie(movie){
  ...
  const img = document.createElement('img')
   ...
   img.src = img_url + movie.poster_path
    const xmarkItem = document.createElement('div')
    xmarkItem.classList.add('xmarkItem')
    const xmark = document.createElement('i')
    xmark.setAttribute('class', "fa-solid fa-xmark")
    xmarkItem.append(xmark)
        ...
        if (el.type == "Trailer"){
            triller.innerHTML = `
               <div class="cont" style="width: 100%">
                 <iframe width="90%"   height="205" src="https://www.youtube.com/embed/${el.key}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
            </div>
        `
        }
        console.log(play)
